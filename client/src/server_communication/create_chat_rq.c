...
uint32_t rq_create_chat(char* name, uint32_t owner_id,
                        bool is_private, char* server_address, int port) {
    ...
    t_packet private = create_packet(PACKET_TYPE_UINT8, &(bool){is_private});
    ...
    send_and_release_packet(client_socket, &private);
